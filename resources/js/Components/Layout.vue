<template>
  <div class="flex h-screen w-full bg-background overflow-hidden relative">
    ...
    <main class="flex-1 flex flex-col h-screen overflow-hidden relative z-0">
      <header class="h-28 px-10 flex items-center justify-between shrink-0">
        <div class="relative w-96">
          <Search class="absolute left-5 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
          <input type="text" placeholder="Search..." class="h-12 w-full rounded-full border-0 bg-card pl-12 pr-4 shadow-[0_4px_20px_rgba(0,0,0,0.03)] ring-1 ring-inset ring-border/30 focus:ring-2 focus:ring-inset focus:ring-primary outline-hidden text-sm font-medium" />
        </div>
        
        <div class="flex items-center gap-6">
          <button class="flex h-12 w-12 items-center justify-center rounded-full bg-card shadow-[0_4px_20px_rgba(0,0,0,0.03)] ring-1 ring-inset ring-border/30 hover:bg-muted/30 transition-colors relative">
            <Bell class="h-5 w-5 text-sidebar-foreground" />
            <span class="absolute top-3 right-3 h-2.5 w-2.5 rounded-full bg-destructive border-2 border-card"></span>
          </button>
